...
from fe_beam.core.solvers import ModalResult, ModalSolver
...
if __name__=='__main__':
    ...
    K, M = nx_beam.get_system_matrices()

    dofs = ['u', 'v', 'w', 'phix', 'phiy', 'phiz']
    fixed_support_bcs = [DirichletBC(nx_beam.start_node, dof) for dof in dofs]
    print(len(fixed_support_bcs))

    modal_solver = ModalSolver()
    modal_result = modal_solver.solve(
        K = K,
        M = M,
        dof_manager=nx_beam._dof_manager,
        dirichlet_bcs=fixed_support_bcs,
        n_modes=2
    )
    print([x/(2*np.pi) for x in modal_result.omegas])
